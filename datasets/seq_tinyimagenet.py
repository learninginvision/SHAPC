...
class SequentialTinyImagenet(ContinualDataset):
    ...
    def get_shapley_data(self):
        ...
        #                        np.array(test_dataset.targets) < self.i + (self.N_CLASSES_PER_TASK-1))
        test_mask = np.logical_and(np.array(test_dataset.targets) >= self.i,
                               np.array(test_dataset.targets) < self.i + 1)
        ...
        # self.i += (self.N_CLASSES_PER_TASK - 1)
        self.i += 1
        return test_loader
