...
class _PyTorchGradient(Explainer):
    ...
    def shap_values(self, X, nsamples=200, ranked_outputs=None, output_rank_order="max", rseed=None, return_variances=False):
        ...
        if ranked_outputs is not None and self.multi_output:
            ...
            if output_rank_order == "custom":
                if type(ranked_outputs) != list:
                    ranked_outputs = [ranked_outputs]
                model_output_ranks_list = []
                for i in ranked_outputs:
                    model_output_ranks = torch.full((model_output_values.shape[0],1), i)
                    model_output_ranks_list.append(model_output_ranks)
                model_output_ranks = torch.cat(model_output_ranks_list, dim=1)
            else:
                model_output_ranks = model_output_ranks[:, :ranked_outputs]
        else:
